...
def generate_build_ninja(
    ...
    if build_config:
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            # Autogenerated REL unit names are only unique within a module.
            # Prefer an explicitly module-scoped source object, while keeping
            # the existing unscoped lookup for DOL and established objects.
            obj = objects.get(f"{link_step.name}/{obj_name}") or objects.get(obj_name)
            if obj is None:
...
def generate_objdiff_config(
    ...
    def add_unit(
        ...
        obj = objects.get(f"{module_name}/{obj_name}") or objects.get(obj_name)
        if obj is None:
