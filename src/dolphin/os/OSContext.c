typedef int BOOL;
typedef unsigned int u32;
typedef signed int s32;
typedef unsigned short u16;

typedef struct OSContext OSContext;
struct OSContext {
    u32 gpr[32];
    u32 cr;
    u32 lr;
    u32 ctr;
    u32 xer;
    double fpr[32];
    u32 fpscr_pad[2];
    u32 srr0;
    u32 srr1;
    u16 mode;
    u16 state;
    u32 gqr[8];
    double psf[32];
};

extern void OSRestoreInterrupts(register BOOL level);
extern char lbl_80122A58[]; // .data:0x80122A58
extern void __OSSetExceptionHandler(s32 index, void* handler);
extern BOOL OSDisableInterrupts(void);
extern void DBPrintf(char* msg, ...);
extern void OSReport(const char* msg, ...);
extern s32 __cvt_fp2unsigned(register double value);

#define __OSCurrentContextPtr ((OSContext**)0x800000D4)
#define __OSFPUContextPtr     ((OSContext**)0x800000D8)

#pragma push
#pragma force_active on

asm void __OSLoadFPUContext(register u32 unused, register OSContext* fpuContext)
{
    nofralloc
    lhz     r5, 0x1A2(r4)
    clrlwi. r5, r5, 31
    beq     OSLoadFPUContext_return
    lfd     f0, 0x190(r4)
    mtfsf   255, f0
    mfspr   r5, 920
    extrwi. r5, r5, 1, 2
    beq     OSLoadFPUContext_fprs
    psq_l   f0, 0x1C8(r4), 0, 0
    psq_l   f1, 0x1D0(r4), 0, 0
    psq_l   f2, 0x1D8(r4), 0, 0
    psq_l   f3, 0x1E0(r4), 0, 0
    psq_l   f4, 0x1E8(r4), 0, 0
    psq_l   f5, 0x1F0(r4), 0, 0
    psq_l   f6, 0x1F8(r4), 0, 0
    psq_l   f7, 0x200(r4), 0, 0
    psq_l   f8, 0x208(r4), 0, 0
    psq_l   f9, 0x210(r4), 0, 0
    psq_l   f10, 0x218(r4), 0, 0
    psq_l   f11, 0x220(r4), 0, 0
    psq_l   f12, 0x228(r4), 0, 0
    psq_l   f13, 0x230(r4), 0, 0
    psq_l   f14, 0x238(r4), 0, 0
    psq_l   f15, 0x240(r4), 0, 0
    psq_l   f16, 0x248(r4), 0, 0
    psq_l   f17, 0x250(r4), 0, 0
    psq_l   f18, 0x258(r4), 0, 0
    psq_l   f19, 0x260(r4), 0, 0
    psq_l   f20, 0x268(r4), 0, 0
    psq_l   f21, 0x270(r4), 0, 0
    psq_l   f22, 0x278(r4), 0, 0
    psq_l   f23, 0x280(r4), 0, 0
    psq_l   f24, 0x288(r4), 0, 0
    psq_l   f25, 0x290(r4), 0, 0
    psq_l   f26, 0x298(r4), 0, 0
    psq_l   f27, 0x2A0(r4), 0, 0
    psq_l   f28, 0x2A8(r4), 0, 0
    psq_l   f29, 0x2B0(r4), 0, 0
    psq_l   f30, 0x2B8(r4), 0, 0
    psq_l   f31, 0x2C0(r4), 0, 0
OSLoadFPUContext_fprs:
    lfd     f0, 0x090(r4)
    lfd     f1, 0x098(r4)
    lfd     f2, 0x0A0(r4)
    lfd     f3, 0x0A8(r4)
    lfd     f4, 0x0B0(r4)
    lfd     f5, 0x0B8(r4)
    lfd     f6, 0x0C0(r4)
    lfd     f7, 0x0C8(r4)
    lfd     f8, 0x0D0(r4)
    lfd     f9, 0x0D8(r4)
    lfd     f10, 0x0E0(r4)
    lfd     f11, 0x0E8(r4)
    lfd     f12, 0x0F0(r4)
    lfd     f13, 0x0F8(r4)
    lfd     f14, 0x100(r4)
    lfd     f15, 0x108(r4)
    lfd     f16, 0x110(r4)
    lfd     f17, 0x118(r4)
    lfd     f18, 0x120(r4)
    lfd     f19, 0x128(r4)
    lfd     f20, 0x130(r4)
    lfd     f21, 0x138(r4)
    lfd     f22, 0x140(r4)
    lfd     f23, 0x148(r4)
    lfd     f24, 0x150(r4)
    lfd     f25, 0x158(r4)
    lfd     f26, 0x160(r4)
    lfd     f27, 0x168(r4)
    lfd     f28, 0x170(r4)
    lfd     f29, 0x178(r4)
    lfd     f30, 0x180(r4)
    lfd     f31, 0x188(r4)
OSLoadFPUContext_return:
    blr
}

asm void __OSSaveFPUContext(register u32 unused, register u32 unused2,
                            register OSContext* fpuContext)
{
    nofralloc
    lhz     r3, 0x1A2(r5)
    ori     r3, r3, 1
    sth     r3, 0x1A2(r5)
    stfd    f0, 0x090(r5)
    stfd    f1, 0x098(r5)
    stfd    f2, 0x0A0(r5)
    stfd    f3, 0x0A8(r5)
    stfd    f4, 0x0B0(r5)
    stfd    f5, 0x0B8(r5)
    stfd    f6, 0x0C0(r5)
    stfd    f7, 0x0C8(r5)
    stfd    f8, 0x0D0(r5)
    stfd    f9, 0x0D8(r5)
    stfd    f10, 0x0E0(r5)
    stfd    f11, 0x0E8(r5)
    stfd    f12, 0x0F0(r5)
    stfd    f13, 0x0F8(r5)
    stfd    f14, 0x100(r5)
    stfd    f15, 0x108(r5)
    stfd    f16, 0x110(r5)
    stfd    f17, 0x118(r5)
    stfd    f18, 0x120(r5)
    stfd    f19, 0x128(r5)
    stfd    f20, 0x130(r5)
    stfd    f21, 0x138(r5)
    stfd    f22, 0x140(r5)
    stfd    f23, 0x148(r5)
    stfd    f24, 0x150(r5)
    stfd    f25, 0x158(r5)
    stfd    f26, 0x160(r5)
    stfd    f27, 0x168(r5)
    stfd    f28, 0x170(r5)
    stfd    f29, 0x178(r5)
    stfd    f30, 0x180(r5)
    stfd    f31, 0x188(r5)
    mffs    f0
    stfd    f0, 0x190(r5)
    lfd     f0, 0x090(r5)
    mfspr   r3, 920
    extrwi. r3, r3, 1, 2
    beq     OSSaveFPUContext_return
    psq_st  f0, 0x1C8(r5), 0, 0
    psq_st  f1, 0x1D0(r5), 0, 0
    psq_st  f2, 0x1D8(r5), 0, 0
    psq_st  f3, 0x1E0(r5), 0, 0
    psq_st  f4, 0x1E8(r5), 0, 0
    psq_st  f5, 0x1F0(r5), 0, 0
    psq_st  f6, 0x1F8(r5), 0, 0
    psq_st  f7, 0x200(r5), 0, 0
    psq_st  f8, 0x208(r5), 0, 0
    psq_st  f9, 0x210(r5), 0, 0
    psq_st  f10, 0x218(r5), 0, 0
    psq_st  f11, 0x220(r5), 0, 0
    psq_st  f12, 0x228(r5), 0, 0
    psq_st  f13, 0x230(r5), 0, 0
    psq_st  f14, 0x238(r5), 0, 0
    psq_st  f15, 0x240(r5), 0, 0
    psq_st  f16, 0x248(r5), 0, 0
    psq_st  f17, 0x250(r5), 0, 0
    psq_st  f18, 0x258(r5), 0, 0
    psq_st  f19, 0x260(r5), 0, 0
    psq_st  f20, 0x268(r5), 0, 0
    psq_st  f21, 0x270(r5), 0, 0
    psq_st  f22, 0x278(r5), 0, 0
    psq_st  f23, 0x280(r5), 0, 0
    psq_st  f24, 0x288(r5), 0, 0
    psq_st  f25, 0x290(r5), 0, 0
    psq_st  f26, 0x298(r5), 0, 0
    psq_st  f27, 0x2A0(r5), 0, 0
    psq_st  f28, 0x2A8(r5), 0, 0
    psq_st  f29, 0x2B0(r5), 0, 0
    psq_st  f30, 0x2B8(r5), 0, 0
    psq_st  f31, 0x2C0(r5), 0, 0
OSSaveFPUContext_return:
    blr
}

asm void OSSaveFPUContext(register OSContext* fpuContext)
{
    nofralloc
    addi    r5, r3, 0
    b       __OSSaveFPUContext
}

asm void OSSetCurrentContext(register OSContext* context)
{
    nofralloc
    lis     r4, 0x8000
    stw     r3, 0xD4(r4)
    clrlwi  r5, r3, 2
    stw     r5, 0xC0(r4)
    lwz     r5, 0xD8(r4)
    cmpw    r5, r3
    bne     OSSetCurrentContext_disableFPU
    lwz     r6, 0x19C(r3)
    ori     r6, r6, 0x2000
    stw     r6, 0x19C(r3)
    mfmsr   r6
    ori     r6, r6, 2
    mtmsr   r6
    blr
OSSetCurrentContext_disableFPU:
    lwz     r6, 0x19C(r3)
    rlwinm  r6, r6, 0, 19, 17
    stw     r6, 0x19C(r3)
    mfmsr   r6
    rlwinm  r6, r6, 0, 19, 17
    ori     r6, r6, 2
    mtmsr   r6
    isync
    blr
}

asm OSContext * fn_8000BE5C(void)
{
    nofralloc
    lis     r3, 0x8000
    lwz     r3, 0xD4(r3)
    blr
}

asm u32 fn_8000BE68(register OSContext* context)
{
    nofralloc
    stmw    r13, 0x34(r3)
    mfspr   r0, 913
    stw     r0, 0x1A8(r3)
    mfspr   r0, 914
    stw     r0, 0x1AC(r3)
    mfspr   r0, 915
    stw     r0, 0x1B0(r3)
    mfspr   r0, 916
    stw     r0, 0x1B4(r3)
    mfspr   r0, 917
    stw     r0, 0x1B8(r3)
    mfspr   r0, 918
    stw     r0, 0x1BC(r3)
    mfspr   r0, 919
    stw     r0, 0x1C0(r3)
    mfcr    r0
    stw     r0, 0x80(r3)
    mflr    r0
    stw     r0, 0x84(r3)
    stw     r0, 0x198(r3)
    mfmsr   r0
    stw     r0, 0x19C(r3)
    mfctr   r0
    stw     r0, 0x88(r3)
    mfxer   r0
    stw     r0, 0x8C(r3)
    stw     r1, 4(r3)
    stw     r2, 8(r3)
    li      r0, 1
    stw     r0, 0xC(r3)
    li      r3, 0
    blr
}

asm void OSLoadContext(register OSContext* context)
{
    nofralloc
    lis     r4, OSDisableInterrupts@ha
    lwz     r6, 0x198(r3)
    addi    r5, r4, OSDisableInterrupts@l
    cmplw   r6, r5
    ble     OSLoadContext_notInRAS
    lis     r4, OSDisableInterrupts+0xC@ha
    addi    r0, r4, OSDisableInterrupts+0xC@l
    cmplw   r6, r0
    bge     OSLoadContext_notInRAS
    stw     r5, 0x198(r3)
OSLoadContext_notInRAS:
    lwz     r0, 0(r3)
    lwz     r1, 4(r3)
    lwz     r2, 8(r3)
    lhz     r4, 0x1A2(r3)
    rlwinm. r5, r4, 0, 30, 30
    beq     OSLoadContext_notexc
    rlwinm  r4, r4, 0, 31, 29
    sth     r4, 0x1A2(r3)
    lmw     r5, 0x14(r3)
    b       OSLoadContext_misc
OSLoadContext_notexc:
    lmw     r13, 0x34(r3)
OSLoadContext_misc:
    lwz     r4, 0x1A8(r3)
    mtspr   913, r4
    lwz     r4, 0x1AC(r3)
    mtspr   914, r4
    lwz     r4, 0x1B0(r3)
    mtspr   915, r4
    lwz     r4, 0x1B4(r3)
    mtspr   916, r4
    lwz     r4, 0x1B8(r3)
    mtspr   917, r4
    lwz     r4, 0x1BC(r3)
    mtspr   918, r4
    lwz     r4, 0x1C0(r3)
    mtspr   919, r4
    lwz     r4, 0x80(r3)
    mtcrf   255, r4
    lwz     r4, 0x84(r3)
    mtlr    r4
    lwz     r4, 0x88(r3)
    mtctr   r4
    lwz     r4, 0x8C(r3)
    mtxer   r4
    mfmsr   r4
    rlwinm  r4, r4, 0, 17, 15
    rlwinm  r4, r4, 0, 31, 29
    mtmsr   r4
    lwz     r4, 0x198(r3)
    mtsrr0  r4
    lwz     r4, 0x19C(r3)
    mtsrr1  r4
    lwz     r4, 0x10(r3)
    lwz     r3, 0xC(r3)
    rfi
}

asm u32 fn_8000BFC0()
{
    nofralloc
    mr      r3, r1
    blr
}

void OSClearContext(register OSContext* context)
{
    context->mode = 0;
    context->state = 0;
    if (context == *__OSFPUContextPtr) {
        *__OSFPUContextPtr = 0;
    }
}

asm void fn_8000BFEC(register OSContext* context, register u32 pc,
                     register u32 newsp)
{
    nofralloc
    stw     r4, 0x198(r3)
    stw     r5, 4(r3)
    li      r11, 0
    ori     r11, r11, 0x9032
    stw     r11, 0x19C(r3)
    li      r0, 0
    stw     r0, 0x80(r3)
    stw     r0, 0x8C(r3)
    stw     r2, 8(r3)
    stw     r13, 0x34(r3)
    stw     r0, 0xC(r3)
    stw     r0, 0x10(r3)
    stw     r0, 0x14(r3)
    stw     r0, 0x18(r3)
    stw     r0, 0x1C(r3)
    stw     r0, 0x20(r3)
    stw     r0, 0x24(r3)
    stw     r0, 0x28(r3)
    stw     r0, 0x2C(r3)
    stw     r0, 0x30(r3)
    stw     r0, 0x38(r3)
    stw     r0, 0x3C(r3)
    stw     r0, 0x40(r3)
    stw     r0, 0x44(r3)
    stw     r0, 0x48(r3)
    stw     r0, 0x4C(r3)
    stw     r0, 0x50(r3)
    stw     r0, 0x54(r3)
    stw     r0, 0x58(r3)
    stw     r0, 0x5C(r3)
    stw     r0, 0x60(r3)
    stw     r0, 0x64(r3)
    stw     r0, 0x68(r3)
    stw     r0, 0x6C(r3)
    stw     r0, 0x70(r3)
    stw     r0, 0x74(r3)
    stw     r0, 0x78(r3)
    stw     r0, 0x7C(r3)
    stw     r0, 0x1A4(r3)
    stw     r0, 0x1A8(r3)
    stw     r0, 0x1AC(r3)
    stw     r0, 0x1B0(r3)
    stw     r0, 0x1B4(r3)
    stw     r0, 0x1B8(r3)
    stw     r0, 0x1BC(r3)
    stw     r0, 0x1C0(r3)
    b       OSClearContext
}

asm void OSDumpContext(register OSContext* context)
{
    nofralloc
    mflr    r0
    stw     r0, 4(r1)
    stwu    r1, -0x2F8(r1)
    stmw    r25, 0x2DC(r1)
    mr      r28, r3
    lis     r3, lbl_80122A58@ha
    crxor   6, 6, 6
    addi    r31, r3, lbl_80122A58@l
    mr      r3, r31
    mr      r4, r28
    bl      OSReport
    li      r25, 0
    slwi    r0, r25, 2
    add     r27, r28, r0
    b       Dump_gprCheck
Dump_gprCheck:
    b       Dump_gprCheck2
Dump_gprCheck2:
    b       Dump_gprBody
Dump_gprBody:
    lwz     r8, 0x40(r27)
    mr      r4, r25
    lwz     r5, 0(r27)
    addi    r3, r31, 0x44
    crxor   6, 6, 6
    mr      r6, r5
    mr      r9, r8
    addi    r7, r25, 0x10
    bl      OSReport
    addi    r27, r27, 4
    addi    r25, r25, 1
    cmplwi  r25, 0x10
    blt     Dump_gprBody
    lwz     r4, 0x84(r28)
    addi    r3, r31, 0x74
    lwz     r5, 0x80(r28)
    crxor   6, 6, 6
    bl      OSReport
    lwz     r4, 0x198(r28)
    addi    r3, r31, 0xA4
    lwz     r5, 0x19C(r28)
    crxor   6, 6, 6
    bl      OSReport
    addi    r3, r31, 0xD4
    crxor   6, 6, 6
    bl      OSReport
    li      r25, 0
    slwi    r0, r25, 2
    add     r27, r28, r0
    b       Dump_gqrCheck
Dump_gqrCheck:
    b       Dump_gqrCheck2
Dump_gqrCheck2:
    b       Dump_gqrBody
Dump_gqrBody:
    lwz     r5, 0x1A4(r27)
    mr      r4, r25
    lwz     r7, 0x1B4(r27)
    addi    r3, r31, 0xE8
    addi    r6, r25, 4
    crxor   6, 6, 6
    bl      OSReport
    addi    r27, r27, 4
    addi    r25, r25, 1
    cmplwi  r25, 4
    blt     Dump_gqrBody
    lhz     r0, 0x1A2(r28)
    clrlwi  r0, r0, 31
    cmplwi  r0, 0
    beq     Dump_afterFPU
    bl      OSDisableInterrupts
    lis     r6, 0x8000
    lwz     r0, 0xD4(r6)
    li      r5, 0
    addi    r4, r1, 0x10
    sth     r5, 0x1B0(r1)
    mr      r30, r0
    mr      r29, r3
    sth     r5, 0x1B2(r1)
    lwz     r0, 0xD8(r6)
    cmplw   r4, r0
    bne     Dump_setTmpCtx
    stw     r5, 0xD8(r6)
Dump_setTmpCtx:
    addi    r3, r1, 0x10
    bl      OSSetCurrentContext
    addi    r3, r31, 0x10C
    crxor   6, 6, 6
    bl      OSReport
    li      r25, 0
    slwi    r0, r25, 3
    add     r26, r28, r0
    b       Dump_fprCheck
Dump_fprCheck:
    b       Dump_fprCheck2
Dump_fprCheck2:
    b       Dump_fprBody
Dump_fprBody:
    lfd     f1, 0x98(r26)
    bl      __cvt_fp2unsigned
    mr      r27, r3
    lfd     f1, 0x90(r26)
    bl      __cvt_fp2unsigned
    mr      r5, r3
    crxor   6, 6, 6
    mr      r4, r25
    mr      r7, r27
    addi    r3, r31, 0x120
    addi    r6, r25, 1
    bl      OSReport
    addi    r26, r26, 0x10
    addi    r25, r25, 2
    cmplwi  r25, 0x20
    blt     Dump_fprBody
    addi    r3, r31, 0x13C
    crxor   6, 6, 6
    bl      OSReport
    li      r25, 0
    slwi    r0, r25, 3
    add     r26, r28, r0
    b       Dump_psfCheck
Dump_psfCheck:
    b       Dump_psfCheck2
Dump_psfCheck2:
    b       Dump_psfBody
Dump_psfBody:
    lfd     f1, 0x1D0(r26)
    bl      __cvt_fp2unsigned
    mr      r27, r3
    lfd     f1, 0x1C8(r26)
    bl      __cvt_fp2unsigned
    mr      r5, r3
    crxor   6, 6, 6
    mr      r4, r25
    mr      r7, r27
    addi    r3, r31, 0x150
    addi    r6, r25, 1
    bl      OSReport
    addi    r26, r26, 0x10
    addi    r25, r25, 2
    cmplwi  r25, 0x20
    blt     Dump_psfBody
    li      r5, 0
    sth     r5, 0x1B0(r1)
    lis     r3, 0x8000
    addi    r4, r1, 0x10
    sth     r5, 0x1B2(r1)
    lwz     r0, 0xD8(r3)
    cmplw   r4, r0
    bne     Dump_restoreCtx
    stw     r5, 0xD8(r3)
Dump_restoreCtx:
    mr      r3, r30
    bl      OSSetCurrentContext
    mr      r3, r29
    bl      OSRestoreInterrupts
Dump_afterFPU:
    addi    r3, r31, 0x170
    crxor   6, 6, 6
    bl      OSReport
    lwz     r25, 4(r28)
    li      r26, 0
    b       Dump_bcPre
Dump_bcPre:
    b       Dump_bcSkip
Dump_bcSkip:
    b       Dump_bcCheck
Dump_bcBody:
    lwz     r5, 0(r25)
    mr      r4, r25
    lwz     r6, 4(r25)
    addi    r3, r31, 0x198
    crxor   6, 6, 6
    bl      OSReport
    lwz     r25, 0(r25)
Dump_bcCheck:
    cmplwi  r25, 0
    beq     Dump_exit
    addis   r0, r25, 1
    cmplwi  r0, 0xFFFF
    beq     Dump_exit
    cmplwi  r26, 0x10
    addi    r26, r26, 1
    blt     Dump_bcBody
Dump_exit:
    lmw     r25, 0x2DC(r1)
    lwz     r0, 0x2FC(r1)
    addi    r1, r1, 0x2F8
    mtlr    r0
    blr
}

asm void OSSwitchFPUContext(register s32 exception, register OSContext* context)
{
    nofralloc
    mfmsr   r5
    ori     r5, r5, 0x2000
    mtmsr   r5
    isync
    lwz     r5, 0x19C(r4)
    ori     r5, r5, 0x2000
    mtsrr1  r5
    lis     r3, 0x8000
    lwz     r5, 0xD8(r3)
    stw     r4, 0xD8(r3)
    cmpw    r5, r4
    beq     OSSwitchFPUContext_restoreAndExit
    cmpwi   r5, 0
    beq     OSSwitchFPUContext_loadNew
    bl      __OSSaveFPUContext
OSSwitchFPUContext_loadNew:
    bl      __OSLoadFPUContext
OSSwitchFPUContext_restoreAndExit:
    lwz     r3, 0x80(r4)
    mtcrf   255, r3
    lwz     r3, 0x84(r4)
    mtlr    r3
    lwz     r3, 0x198(r4)
    mtsrr0  r3
    lwz     r3, 0x88(r4)
    mtctr   r3
    lwz     r3, 0x8C(r4)
    mtxer   r3
    lhz     r3, 0x1A2(r4)
    rlwinm  r3, r3, 0, 31, 29
    sth     r3, 0x1A2(r4)
    lwz     r5, 0x14(r4)
    lwz     r3, 0xC(r4)
    lwz     r4, 0x10(r4)
    rfi
}

void __OSContextInit(void)
{
    __OSSetExceptionHandler(7, OSSwitchFPUContext);
    *__OSFPUContextPtr = 0;
    DBPrintf((char*)0x80122C0C);
}

#pragma pop
